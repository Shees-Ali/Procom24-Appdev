  ...
  getFirstFormError(form: FormGroup) {
    ...
    if (first.error?.['minlength']) {
      errorMessage =
        label +
        ' at least ' +
        first.error?.['minlength'].requiredLength +
        ' characters';
    }
    ...
    if (first.error?.['maxlength']) {
      errorMessage =
        label +
        ' at most ' +
        first.error?.['maxlength'].requiredLength +
        ' characters';
    }
    ...
    if (first.error?.['pattern']) {
      errorMessage =
        label +
        ' should be like ' +
        first.error?.['pattern'].requiredPattern +
        ' pattern';
    }

    if (first.error?.['email']) {
      errorMessage = label + ' should be like abc@email.com';
    }
